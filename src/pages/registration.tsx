...
const Registration = () => {
    ...
    const register = ()=>{
        if (username.current?.value && password.current?.value && confirmed_password.current?.value)
        reg.mutate({username:username.current.value,password:password.current.value,confirmed_password:confirmed_password.current.value})
    }
    if (reg.isError){alert(reg.error.message)}
    if (reg.isSuccess){window.location.href='/login'}
  return (
    <div>
      <NavMenu />
      <div>username :</div>
